...
export default function Home() {
  ...
  return (
    <main className="shell">
      ...
      <section className="trend panel"><div className="panel-head"><div><p className="eyebrow">MODEL HISTORY</p><h2>Published probability history</h2></div><span className="help">released baselines · not actuals</span></div><p className="trend-copy">This records what the published baseline calculated at each model release: the probability of expansion, compression or a mixed transition. It does not use a made-up growth number, and it does not change when a visitor adjusts a scenario.</p><div className="trend-chart"><svg viewBox="0 0 720 190" preserveAspectRatio="none"><line x1="45" y1="20" x2="45" y2="155" /><line x1="45" y1="155" x2="690" y2="155" /><polyline className="expansion-line" points="70,109 650,106" /><polyline className="compression-line" points="70,85 650,86" /><polyline className="mixed-line" points="70,137 650,137" /><circle className="expansion-point" cx="70" cy="109" r="5" /><circle className="expansion-point" cx="650" cy="106" r="5" /><circle className="compression-point" cx="70" cy="85" r="5" /><circle className="compression-point" cx="650" cy="86" r="5" /><circle className="mixed-point" cx="70" cy="137" r="5" /><circle className="mixed-point" cx="650" cy="137" r="5" /></svg><div className="trend-labels"><span><i className="trend-key expansion-key" />Expansion <b>34% → 36%</b></span><span><i className="trend-key compression-key" />Compression <b>52% → 51%</b></span><span><i className="trend-key mixed-key" />Mixed transition <b>13% → 13%</b></span></div></div><div className="release-list">{modelHistory.map((release) => <div key={release.version}><b>{release.date} · v{release.version}</b><span><em>{release.changeType}</em> · {release.note}</span>{release.updates && <ul className="release-updates">{release.updates.map((update) => <li key={update.label}><b>{update.label}</b><em className={`effect effect-${update.effect.startsWith("Positive") ? "positive" : update.effect.startsWith("Mixed") ? "mixed" : "uncertain"}`}>{update.effect}</em><span>{update.detail}</span></li>)}</ul>}<strong>Expansion {release.expansion}% · Compression {release.compression}% · Mixed {release.mixed}%</strong></div>)}</div></section>
